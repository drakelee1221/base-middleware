...
import com.apidocs.code.util.DateUtil;
...
public   class CodeGenerator {
	...
	private void splitColumns( List<Column> lsColumns ) {
		...
		for (Column column : lsColumns) {
			if ("TRUE".equalsIgnoreCase(column.getColumnKey())) {
				...
				tool.put("pkComment", column.getComments().trim());
				tool.put("pkName", column.getName());
			}else{
	...
	public void setTableComment(String comment){
		tool.put("tableComment", comment);
	}

	public void initCodeTool(List<Column> lsColumns) throws Exception {
		classVar = StringUtil.toBeanPatternStr(org.apache.commons.lang.StringUtils.isBlank(tablePrefix)
																					 ? tableName : (tableName.startsWith(tablePrefix) ? tableName.substring(tablePrefix.length()) : tableName));
...
//		tool.put("stringUtil",stringUtil );
	...
	public void createCodeByConf(Map<String, String> params) throws Exception {
		//step 1
		tool.setTmplDir(CodeTool.RESOURCE_PATH_PREFIX+"templates");
		tool.initVelocity();
		//step 2
		for (String key  :params.keySet()) {
			if("driver".equals(key.trim())){
				tool.put("jdbc.driver",params.get(key).trim());
			}else if("url".equals(key.trim())){
				tool.put("jdbc.url",params.get(key).trim());
			}else if("userName".equals(key.trim())){
				tool.put("jdbc.username",params.get(key).trim());
			}else if("password".equals(key.trim())){
				tool.put("jdbc.password",params.get(key).trim());
			}else{
				tool.put(key.trim(),params.get(key).trim());
			}
		}
		//step 3
		String mybatis = params.get("mybatis");
		Properties properties = new PropertiesUtil().load("templates-jpa.cfg");
		if(!StringUtils.isEmpty(mybatis)){
			properties = new PropertiesUtil().load("templates-mybatis.cfg");
		}
